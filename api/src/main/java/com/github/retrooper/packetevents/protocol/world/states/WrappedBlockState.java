...
public class WrappedBlockState {
    ...
    private static final WrappedBlockState[][] BY_ID = new WrappedBlockState[HIGHEST_MAPPING_INDEX + 1][];
    ...
    static {
        ...
        BY_ID[AIR_MAPPING_INDEX] = new WrappedBlockState[AIR.getGlobalId() + 1];
        BY_ID[AIR_MAPPING_INDEX][AIR.getGlobalId()] = AIR;
    ...
    private static byte loadMappings(ClientVersion version) {
        ...
        if (!PRELOAD_BLOCK_STATE_MAPPINGS && BY_ID[mappingsIndex] == null) {
    ...
    private static Map<Map<StateValue, Object>, StateCacheValue> buildStateDataCache() {
        ...
        for (byte i = 0; i < HIGHEST_MAPPING_INDEX; i++) {
            Map<String, WrappedBlockState> map = BY_STRING[i];
            if (map == null) {
    ...
    public static WrappedBlockState getByGlobalId(ClientVersion version, int globalID, boolean clone) {
        ...
        final WrappedBlockState state = BY_ID[mappingsIndex][globalID];
        return state == null ? AIR : (clone ? state.clone() : state);
    }
        ...
        try (final SequentialNBTReader.Compound compound = MappingHelper.decompress(MAPPINGS_ASSETS_LEGACY)) {
            ...
            for (Map.Entry<String, NBT> entry : (SequentialNBTReader.Compound) compound.next().getValue()) {
                ...
                for (Map.Entry<String, NBT> element : inner) {
                    ...
                    maxCombinedId = Math.max(maxCombinedId, combinedID);
            ...
            BY_ID[LEGACY_MAPPING_INDEX] = new WrappedBlockState[maxCombinedId + 1];
            for (Map.Entry<Integer, WrappedBlockState> entry : stateByIdMap.entrySet()) {
                BY_ID[LEGACY_MAPPING_INDEX][entry.getKey()] = entry.getValue();
            }
    ...
    private void checkIsStillValid() {
        ...
        if (globalID == -1) { // -1 maps to no block as negative ID are impossible
            WrappedBlockState blockState = BY_ID[this.mappingsIndex][oldGlobalID];
            blockState = blockState == null ? AIR : blockState.clone();
